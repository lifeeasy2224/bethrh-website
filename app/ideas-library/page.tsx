    ...
    >
      <div className="px-5 pt-5 pb-3">
        <div className="flex items-center justify-between flex-row-reverse mb-3">
          <span className="text-xs font-semibold px-2.5 py-1 rounded-full flex items-center gap-1.5 shrink-0" style={{ background: 'hsl(144,58%,93%)', color: 'var(--green-deep)' }}>
            <span>{seed.sector_emoji}</span>{seed.sector_label}
          </span>
          <span
        ...
        <h3 className="font-bold text-base leading-snug mb-2 transition-colors group-hover:opacity-80" style={{ color: 'var(--text-dark)' }}>{seed.name}</h3>
        {seed.short_description && (
          <p className="text-xs leading-relaxed line-clamp-3" style={{ color: 'var(--gray-mid)' }}>{seed.short_description}</p>
        )}
      </div>
      ...
      <div className="mx-5 mb-4 grid grid-cols-3 gap-2">
        <div className="rounded-xl p-2.5 text-center" style={{ background: 'hsl(42,30%,96%)' }}>
          ...
          <div className="text-xs font-bold leading-tight" style={{ color: 'hsl(42,55%,28%)' }}>${formatUSD(seed.investment_min)}+</div>
      ...
      <div className="px-5 pb-5 mt-auto">
        ...
        >
          {isFull ? 'مكتملة' : <><Sprout className="w-3.5 h-3.5" />اعرف أكثر <ChevronLeft className="w-3.5 h-3.5" /></>}
        </div>
      </div>
    </Link>
  );
  ...
  return (
    <div dir="rtl" className="min-h-screen" style={{ background: 'hsl(42,25%,97%)' }}>
      ...
      <section className="pt-14 pb-10 text-center px-6">
        <span className="inline-flex items-center gap-1.5 text-xs font-bold px-3 py-1.5 rounded-full mb-5" style={{ background: 'hsl(144,58%,92%)', color: 'hsl(144,58%,26%)' }}>
          <Sprout className="w-3.5 h-3.5" />بذور جاهزة للزراعة
        </span>
        <h1 className="text-4xl md:text-5xl font-extrabold mb-4" style={{ color: 'hsl(158,30%,14%)' }}>مكتبة الأفكار 🌱</h1>
        <p className="text-lg max-w-lg mx-auto mb-2" style={{ color: 'hsl(158,20%,30%)' }}>أفكار مدروسة جاهزة للتنفيذ — اختر فكرتك وابدأ رحلتك</p>
        <p className="text-sm max-w-sm mx-auto mb-8" style={{ color: 'hsl(158,20%,44%)' }}>تصفّح مجاناً — سجّل لالتقاط فكرة وإضافتها لمشروعك</p>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-8 space-y-4">
        <div className="relative max-w-md">
          ...
          <input type="text" value={search} onChange={e => setSearch(e.target.value)} placeholder="ابحث عن فكرة..." className="w-full pr-10 pl-4 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2" style={{ borderColor: 'var(--gray-light)', background: 'var(--white)' }} dir="rtl" />
        </div>
        <div className="flex gap-2 flex-wrap flex-row-reverse">
          {SECTORS.map(s => {
            ...
            return (
              <button key={s.id} onClick={() => setActiveSector(s.id)} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium transition-all" style={isActive ? { background: 'hsl(144,58%,22%)', color: 'white' } : { background: 'white', border: '1px solid hsl(42,25%,84%)', color: 'hsl(158,20%,32%)' }}>
                <span>{s.emoji}</span><span>{s.label}</span>
                <span className="text-[10px] px-1.5 py-0.5 rounded-full font-semibold" style={isActive ? { background: 'hsl(144,58%,36%)', color: 'white' } : { background: 'hsl(42,25%,92%)', color: 'hsl(158,20%,36%)' }}>{count}</span>
              </button>
            );
          })}
        </div>
        {!loading && <p className="text-xs text-muted-foreground text-right">{filtered.length} فكرة</p>}
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-20">
        {loading ? (
          <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-5">
            {Array.from({ length: 6 }).map((_, i) => <div key={i} className="h-72 rounded-2xl animate-pulse" style={{ background: 'hsl(42,25%,92%)' }} />)}
          </div>
        ) : filtered.length === 0 ? (
      ...
      <section className="py-16 text-center px-6" style={{ background: 'var(--green-deep)', color: 'var(--white)' }}>
        ...
        <p className="text-sm mb-6 max-w-xs mx-auto" style={{ color: 'var(--gold)' }}>سجّل مجاناً، التقط فكرة، وابدأ رحلتك الريادية اليوم.</p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link href="/signup" className="inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl text-sm font-bold shadow-md hover:opacity-90 transition-all flex-row-reverse" style={{ background: 'var(--gold)', color: 'var(--green-deep)' }}>
            ابدأ مجاناً <ArrowLeft className="w-4 h-4" />
          </Link>
          <Link href="/pricing" className="inline-flex items-center justify-center px-8 py-3.5 rounded-xl text-sm font-semibold border transition-all hover:bg-white/10" style={{ borderColor: 'var(--green-brand)', color: 'var(--green-brand)' }}>
            شاهد الباقات
          </Link>
        </div>
      </section>
