...
const STARTER_PROMPTS = [
  'كيف أتحقق من فكرة مشروعي في سوريا؟',
  'ما أفضل القطاعات لرواد الأعمال السوريين؟',
...
function buildMockResponse(message: string): string {
  ...
  if (lower.includes('تحقق') || lower.includes('فكرة') || lower.includes('validate')) {
    return 'سؤال ممتاز! للتحقق من فكرتك في السياق السوري، ابدأ بـ 10 مقابلات مع عملاء محتملين قبل البناء. ركّز على: (١) هل هذه مشكلة حقيقية؟ (٢) كيف يحلونها اليوم؟ (٣) هل سيدفعون مقابل حل أفضل؟ استهدف الحصول على 3 طلبات مسبقة أو 20 تسجيلاً كأول هدف. سجّل كل شيء في متتبع التحقق هنا.';
  }
  if (lower.includes('قطاع') || lower.includes('زراعي') || lower.includes('غذاء')) {
    return 'القطاعات الأربعة الرئيسية ذات الفرص الكبيرة: **الغذاء الزراعي** (تصنيع، تعبئة، علامات تجارية محلية)، **التكنولوجيا الزراعية** (تقنيات الري، مراقبة المحاصيل)، **الخدمات** (التعليم، الرعاية الصحية، الخدمات المهنية)، و**التصدير** (زيت الزيتون، الحرف، المنسوجات). أي قطاع يثير اهتمامك أكثر؟';
  }
  if (lower.includes('عميل') || lower.includes('أول')) {
    return 'للعثور على أول 10 عملاء: (١) ابدأ بشبكتك الشخصية — من يعاني من المشكلة التي تحلها؟ (٢) انشر في مجموعات فيسبوك ذات الصلة بقطاعك. (٣) زر 3 أسواق أو شركات محلية شخصياً. (٤) اطلب من كل عميل إحالة واحدة. الهدف ليس التوسع بعد — بل التعلم. ما هي فكرتك؟';
  }
  if (lower.includes('عرض القيمة') || lower.includes('value')) {
    return 'عرض القيمة القوي يتبع هذه الصيغة: **"نساعد [نوع العميل] الذي [يعاني من هذه المشكلة] على [تحقيق هذه النتيجة] بخلاف [البدائل الحالية]."** مثال: "نساعد مزارعي الزيتون السوريين على زيادة إنتاجهم بنسبة 20% عبر تنبيهات SMS." ما هو نوع عميلك ومشكلته؟';
  }
  if (lower.includes('إيرادات') || lower.includes('تسعير') || lower.includes('نموذج')) {
    return 'لمرحلة الانطلاق، فكّر في هذه النماذج: (١) **المبيعات المباشرة** — مناسب للغذاء الزراعي. (٢) **الاشتراك** — رسوم شهرية للقيمة المستمرة. (٣) **العمولة** — نسبة من المعاملات، مخاطر أقل. ابدأ بما يحتاج أقل قدر من بناء الثقة. ما نموذج عملك؟';
  }
      ...
      <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-card shrink-0 flex-row-reverse">
        <div className="flex items-center gap-3 flex-row-reverse">
          ...
          <div className="text-right">
            ...
            <p className="text-xs text-muted-foreground">متخصص في السياق السوري</p>
          </div>
        </div>
        {messages.length > 0 && (
