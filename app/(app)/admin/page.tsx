...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    fetchStats();
  }, [isAdmin, authLoading, fetchStats]);

  if (authLoading || (fetching && !stats)) {
    return (
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6" dir="rtl">
            ...
            {['اليوم', '7 أيام', '30 يوم', 'كل الوقت'].map(range => (
              <button key={range} onClick={() => setDateRange(range)}
                className={`px-3 py-1.5 rounded-lg text-sm font-medium transition-all ${
                  dateRange === range ? 'bg-foreground text-background' : 'bg-secondary text-muted-foreground hover:text-foreground'
                }`}>
                {range}
              </button>
            ))}
      ...
      <div>
        <h2 className="text-sm font-semibold text-muted-foreground mb-3 text-right flex items-center gap-2 flex-row-reverse">
          <Sprout className="w-4 h-4" /> إحصائيات المشتل 🪴
        </h2>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-border">
              {s.recentIdeas.length === 0 ? (
                <tr><td colSpan={3} className="py-6 text-center text-muted-foreground">لا توجد أفكار بعد</td></tr>
